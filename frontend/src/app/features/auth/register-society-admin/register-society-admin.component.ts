...
@Component({
  selector: 'app-register-society-admin',
  standalone: true,
  ...
  templateUrl: './register-society-admin.component.html',
  styleUrl: './register-society-admin.component.scss'
})
export class RegisterSocietyAdminComponent implements OnInit {
  ...
  form = { fullName: '', email: '', phoneNumber: '', password: '', societyId: '' };
  ...
  submit(): void {
    if (!this.form.fullName || !this.form.email || !this.form.phoneNumber || !this.form.password || !this.form.societyId) return;
    ...
    this.authService.registerSocietyAdmin({ ...this.form, profileImageUrl: this.uploadedPhotoUrl() ?? undefined }).subscribe({
      next: () => {
